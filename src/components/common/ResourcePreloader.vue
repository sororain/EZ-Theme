...
export default {
  ...
  setup() {
    ...
    const schedulePreload = (callback, timeout) => {
      if (document.hidden) {
        return scheduleTimeout(() => {
          schedulePreload(callback, timeout);
        }, 1500);
      }
    ...
    const getRouteComponents = (path) => {
      if (!(path in componentsConfig.route)) {
        return [];
      }

      const routeComponents = [...componentsConfig.route[path]];

      if (
        isCustomerServiceEnabled &&
        path !== '/customer-service' &&
        !routeComponents.some(comp => comp.name === 'CustomerService')
      ) {
        routeComponents.unshift(customerServiceComponent);
      }
    ...
    const getBaseComponents = () => {
      const baseComponents = [...componentsConfig.base];

      if (
        isCustomerServiceEnabled &&
        !baseComponents.some(comp => comp.name === 'CustomerService')
      ) {
        baseComponents.unshift(customerServiceComponent);
      }
    ...
    const preloadVueComponents = () => {
      ...
      enqueueComponents(getRouteComponents(route.path));

      scheduleTimeout(() => {
        enqueueComponents(getBaseComponents());
      }, 8000);
    ...
    watch(() => route.path, (newPath, oldPath) => {
      if (newPath !== oldPath && newPath in componentsConfig.route) {
        enqueueComponents(getRouteComponents(newPath));
      }
    });
